...
import { Sparkles, History, MessageCircle, BookOpen, FileText, LayoutDashboard, TrendingUp } from "lucide-react"
...
const navigation = [
  ...
  { name: "Trending Narratives", href: "/app/trending", icon: TrendingUp },
  { name: "Generate Tokens", href: "/app/generate", icon: Sparkles },
  { name: "Generate Content", href: "/app/content", icon: FileText },
  { name: "History Tokens", href: "/app/history", icon: History },
]
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <item.icon
                className={cn("w-5 h-5", isActive ? "text-white" : "text-muted-foreground group-hover:text-green-600")}
                strokeWidth={2}
              />
              <span>{item.name}</span>
            </Link>
